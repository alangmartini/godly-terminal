<#
.SYNOPSIS
  OMX Visual Parity Loop - autonomous native visual-parity loop for godly-shell.

.DESCRIPTION
  Repeatedly invokes omx exec non-interactively with a repo-specific parity prompt
...
param(
    ...
    [string]$DiffImage = (Join-Path $PSScriptRoot "docs\references\current-godly-shell.diff.png"),
...
$DiffImage = [System.IO.Path]::GetFullPath($DiffImage)
$TaskContractPath = [System.IO.Path]::GetFullPath($TaskContractPath)
...
function Enable-CodexCmdShim {
    ...
    $shimDir = Join-Path $ShimRoot ".shim-bin"
...
function Get-AttachedImages {
    $images = @()
    foreach ($candidate in @($WebRefImage, $NativeImage, $DiffImage)) {
...
function Read-TaskContract {
    if (-not (Test-Path -LiteralPath $TaskContractPath)) {
        throw "Task contract not found: $TaskContractPath"
    }

    return [System.IO.File]::ReadAllText($TaskContractPath)
}
...
function Write-RunManifest {
    $manifest = [ordered]@{
        ...
        attached_images = @(Get-AttachedImages)
...
function Get-PromptBody {
    ...
    $attachedImages = Get-AttachedImages
    $taskContract = Read-TaskContract
    $imageSection = if ($attachedImages.Count -gt 0) {
        $attachedList = $attachedImages | ForEach-Object { "- $_" }
        @"
...
"@
        ...
        "No screenshots are attached for this iteration.`n`n"
    }
    ...
    return @"
OMX Visual Parity Loop iteration $Iteration.
...
Diff screenshot target: $DiffImage
$previousOutputNote
...
function Write-PromptFile {
    ...
    $content = Get-PromptBody -Iteration $Iteration -PreviousLastMessagePath $PreviousLastMessagePath
...
function Invoke-AgentIteration {
    ...
    foreach ($image in $attachedImages) {
        $args += @("-i", $image)
    }
    ...
    $promptText | & $AgentCommand @args 2>&1 | ForEach-Object {
        $line = [string]$_
...
$codexShim = Enable-CodexCmdShim -ShimRoot $ArtifactsRoot
Write-RunManifest

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  OMX Visual Parity Loop" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  Diff:      $DiffImage"
Write-Host "Iterations:  $(if ($MaxIterations -eq 0) { 'unlimited' } else { $MaxIterations })"
Write-Host ""

if ($DryRun) {
    $promptPath = Join-Path $RunDir "dry-run-prompt.md"
    Write-PromptFile -Iteration 1 -PromptPath $promptPath -PreviousLastMessagePath $null
    Write-Host "[OMX Visual Parity Loop] Dry run complete. Prompt written to $promptPath" -ForegroundColor Yellow
    Write-Host "[OMX Visual Parity Loop] Attached images: $((Get-AttachedImages) -join ', ')" -ForegroundColor Yellow
    Write-Host "[OMX Visual Parity Loop] Command preview: $AgentCommand exec --dangerously-bypass-approvals-and-sandbox --json --color never -C `"$ProjectRoot`" -o <last-message> -m $Model -c model_reasoning_effort=`"$ReasoningEffort`" -" -ForegroundColor Yellow
    if (-not $NoCleanup) {
        Write-Host "[OMX Visual Parity Loop] Cleaning up..." -ForegroundColor DarkGray
        Stop-ViteProcesses
    }
    if ($codexShim) {
        $env:PATH = $codexShim.previous_path
    }
    Write-Host "[OMX Visual Parity Loop] Finished after 0 iterations." -ForegroundColor Green
    Write-Host "[OMX Visual Parity Loop] Run artifacts saved to $RunDir" -ForegroundColor Green
    exit 0
}
...
while ($true) {
    if (Test-Path $StopFile) {
        Write-Host ""
        Write-Host "[OMX Visual Parity Loop] Stop file detected. Exiting before next iteration." -ForegroundColor Yellow
        break
    }
    ...
    if ($MaxIterations -gt 0 -and $iteration -gt $MaxIterations) {
        Write-Host ""
        Write-Host "[OMX Visual Parity Loop] Reached max iterations ($MaxIterations). Stopping." -ForegroundColor Yellow
        break
    }
    ...
    Write-Host "[OMX Visual Parity Loop] Prompt:      $promptPath" -ForegroundColor DarkGray
    Write-Host "[OMX Visual Parity Loop] Transcript: $transcriptPath" -ForegroundColor DarkGray
    Write-Host "[OMX Visual Parity Loop] Events:     $eventsPath" -ForegroundColor DarkGray
    Write-Host "[OMX Visual Parity Loop] Final msg:  $lastMessagePath" -ForegroundColor DarkGray
    ...
    try {
        ...
        if ($doneText -match "(?m)^RALPH_DONE\b") {
            Write-Host ""
            Write-Host "[OMX Visual Parity Loop] Parity achieved. Stopping." -ForegroundColor Green
            break
        }
        ...
        Write-Host "[OMX Visual Parity Loop] Error in iteration ${iteration}: $_" -ForegroundColor Red
        Write-Host "[OMX Visual Parity Loop] Retrying in $RetryDelaySeconds seconds..." -ForegroundColor Yellow
    ...
    if (Test-Path $StopFile) {
        Write-Host ""
        Write-Host "[OMX Visual Parity Loop] Stop file detected. Exiting after current iteration." -ForegroundColor Yellow
        break
    }

    Write-Host ""
    Write-Host "[OMX Visual Parity Loop] Iteration $iteration complete. Starting next in $IterationDelaySeconds seconds..." -ForegroundColor Cyan
...
Write-Host "[OMX Visual Parity Loop] Finished after $iteration iterations." -ForegroundColor Green
Write-Host "[OMX Visual Parity Loop] Run artifacts saved to $RunDir" -ForegroundColor Green
