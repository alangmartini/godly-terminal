...
fn request_all_variants_serialize() {
    let variants: Vec<Request> = vec![
        ...
        Request::SetScrollback {
            session_id: "s".into(),
            offset: 5,
        },
        Request::ScrollAndReadRichGrid {
            session_id: "s".into(),
            offset: 10,
        },
    ...
    assert_eq!(
        variants.len(),
        19,
        "Update when adding new Request variants"
    );
}
...
fn response_all_variants_serialize() {
    let variants: Vec<Response> = vec![
        ...
        Response::RichGrid {
            grid: make_test_rich_grid(),
        },
        Response::RichGridDiff {
            diff: make_test_rich_grid_diff(),
        },
        Response::GridText {
    ...
    assert_eq!(
        variants.len(),
        12,
        "Update when adding new Response variants"
    );
}
